...
class PresenceService:
    ...
    async def _publish_presence(self, user_id: int, guild_id: int, status: str):
        """Publish presence update to Redis (lazy import to avoid circular dependency)"""
        from app.events.publisher import redis_publisher
        await redis_publisher.publish_user_presence(user_id, guild_id, status)
    
    async def _publish_typing(self, channel_id: int, user_id: int, username: str, action: str):
        """Publish typing indicator to Redis (lazy import to avoid circular dependency)"""
        from app.events.publisher import redis_publisher
        await redis_publisher.publish_typing_indicator(channel_id, user_id, username, action)
    ...
    async def set_user_online(self, user_id: int, guild_id: int, status: str = "online") -> None:
        ...
        # Publish to Redis for cross-instance updates
        await self._publish_presence(user_id, guild_id, "online")
    ...
    async def set_user_offline(self, user_id: int, guild_id: Optional[int] = None) -> None:
        ...
        if guild_id:
            ...
            # Publish to Redis for cross-instance updates
            await self._publish_presence(user_id, guild_id, "offline")
        else:
            user_guilds_key = f"presence:user:{user_id}:guilds"
            ...
            for gid in guilds:
                ...
                await self.redis.srem(guild_key, user_id)
                await self._publish_presence(user_id, int(gid), "offline")
    ...
    async def start_typing(self, user_id: int, username: str, channel_id: int, guild_id: int) -> None:
        ...
        # Publish to Redis for cross-instance updates
        await self._publish_typing(channel_id, user_id, username, "start")
    ...
    async def stop_typing(self, user_id: int, channel_id: int) -> None:
        ...
        # Publish to Redis for cross-instance updates
        from app.events.publisher import redis_publisher
        await self._publish_typing(channel_id, user_id, "", "stop")
